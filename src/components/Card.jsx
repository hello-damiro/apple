...
export default function Card({
  ...
  const justify = position === 'center' ? 'center' : `flex-${position}`;
  ...
  return (
    <div className='scale-100 hover:scale-[1.02] transition-all duration-700 pr-6'>
      <Link href={href} className='flex snap-always snap-start md:snap-center scroll-mx-6 scroll-px-6'>
        <div className='relative overflow-hidden w-full rounded-2xl shadow-gray-md shadow-sm hover:shadow-lg transition-all duration-700'>
          {card}
          <div
            className={cn(
